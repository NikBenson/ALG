\section{Implementierung}
	...
	\subsection{Auswahl der Splitter}
		...
		\subsubsection{Zufällig}
			\label{sec:random}
			\lstinputlisting[language=C, caption={Auswahl der Sample Zufällig \autocite{benson-2022}}, label=fig:select_sample_random]{../code/src/samplesort/choose_sample/random.h}
			...
			Im durchschnitt werden so genau die mittleren Splitter, die versucht werden, möglichst genau zu treffen, gefunden.
			 Entsprechend ist hier die Standardabweichung der Elemente von $\sqrt{\frac{\displaystyle\sum_{i=1}^n(i-\frac{n}{2})^2}{n-1}}\equiv\frac{\sqrt{\frac{n^3+2n}{n-1}}}{2\sqrt{3}}\approx n$ relevant.\footnote{Zur vereinfachten Darstellung wird angenommen, dass alle Elemente einzigartig sind.}
